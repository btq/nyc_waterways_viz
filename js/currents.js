...
(function() {
    ...
    function createSettings(topo) {
        ...
        var settings = {
            ...
            animate: true,
            styles: styles,
            styleIndex: function(m) {  // map current speed to a style
     ...
     * (x, y) is not masked.
     *
     * The field mask defines the area where the current vector field is available. The field extends beyond the
    ...
    /**
     * Converts a meteorological current vector to a u,v-component vector in pixel space. For example, given current
     ...
     * http://mst.nerc.ac.uk/current_vect_convs.html.
     */
    function componentize(current) {
        var φ = current[0] / 360 * τ;  // meteorological current direction in radians
        var m = current[1];  // current velocity, knots
    ...
    /**
     * Returns a human readable string for the provided rectangular current vector.
     */
    function formatVector(x, y) {
        var d = Math.atan2(-x, y) / τ * 360 +180;  // calculate into-the-current cardinal degrees
    ...
    function drawOverlay(data, settings, masks) {
        ...
        if (points.length < 3) {  // we need at least three samples to interpolate
            return d.reject("Need at least 3 samples to interpolate");
        }
	...
	/*var sometopo = loadJson(displayData.topography);
	console.log(sometopo)
	console.log("Map loaded")
	var somedata = loadJson(displayData.samples)
	console.log("Data loaded")
	var someinit = init();
	//var somecreateS = createSettings(sometopo);
	var somecreaeS = createSettings();
	var somebuildM = buildMeshes(sometopo, somecreateS);
	var somerendr = render(somecreateS, somebuildM);
	var someplotS = plotStations(somedata, somebuildM);
	drawOverlay;
	*/
